...
def Train_fold(i,X_train,y_train,X_val,y_val):
    PLR = LogisticRegressionCV(Cs=1, penalty='l1', fit_intercept=False,  solver='liblinear',refit=True, class_weight='balanced')
    PLR.fit(X_train, y_train)
    y_pred = PLR.predict_proba(X_val)
    AUC = roc_auc_score(y_val[:,0], y_pred[:,0])
    print(AUC)
    joblib.dump(PLR, filename='Cancer'+str(i)+'_PLR.model')
